        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Card className="p-5" data-testid="broker-charges-card">
              <h3 className="font-bold text-gray-800 mb-4 text-base">Broker Charges</h3>
              <div className="space-y-2">
                {[
                  ['Brokerage', report.brokerage],
                  ['STT', report.stt_paid],
                  ['Transaction Charges', report.txn_charges],
                  ['GST (18%)', report.gst],
                ...
                <div className="flex justify-between py-2 bg-red-50 rounded px-2 -mx-2">
                  ...
                  <span className="text-sm font-bold text-red-600">{fmt(report.total_charges || 0)}</span>
                </div>
                <div className="flex justify-between py-2 bg-blue-50 rounded px-2 -mx-2 mt-1">
                  <span className="text-sm font-bold text-gray-800">Net P&L After Charges</span>
                  <span className={`text-sm font-bold ${(report.net_pnl_after_charges || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    {(report.net_pnl_after_charges || 0) >= 0 ? '+' : ''}{fmt(report.net_pnl_after_charges || report.net_pnl)}
                  </span>
                </div>
              </div>
            </Card>
            ...
            <Card className="p-5" data-testid="tax-calculation-card">
              ...
              <div className="space-y-2">
                <div className="flex justify-between py-1.5 border-b border-gray-100">
                  ...
                  <span className={`text-sm font-semibold ${report.net_pnl >= 0 ? 'text-green-600' : 'text-red-600'}`}>{report.net_pnl >= 0 ? '+' : ''}{fmt(report.net_pnl)}</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-gray-100">
                  <span className="text-sm text-gray-600">Less: Charges</span>
                  <span className="text-sm text-red-600">-{fmt(report.total_charges || 0)}</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-gray-100">
                  <span className="text-sm font-semibold text-gray-700">Taxable Income</span>
                  <span className={`text-sm font-bold ${(report.net_pnl_after_charges || report.net_pnl) >= 0 ? 'text-green-600' : 'text-red-600'}`}>{fmt(report.net_pnl_after_charges || report.net_pnl)}</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-gray-100">
