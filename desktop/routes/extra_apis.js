...
module.exports = function (db) {
  ...
  router.post('/api/option-chain/greeks', (req, res) => {
    const { spot = 0, strike = 0, days_to_expiry = 7, iv = 20, option_type = 'CE' } = req.body || {};
    if (!spot || !strike) return res.json({ status: 'error', message: 'spot and strike required' });
    const T = Math.max(days_to_expiry / 365, 1 / 365);
    const r = 0.07;
    const sigma = Math.max(iv, 1) / 100;
    const price = option_type === 'CE' ? blackScholesCall(spot, strike, T, r, sigma) : blackScholesPut(spot, strike, T, r, sigma);
    res.json({
      status: 'success', spot, strike, days_to_expiry, iv, option_type,
      price: Math.round(price * 100) / 100,
      delta: calcDelta(spot, strike, T, r, sigma, option_type),
      gamma: calcGamma(spot, strike, T, r, sigma),
      theta: calcTheta(spot, strike, T, r, sigma, option_type),
      vega: calcVega(spot, strike, T, r, sigma),
    });
  });

  router.post('/api/option-chain/iv', (req, res) => {
    const { market_price = 0, spot = 0, strike = 0, days_to_expiry = 7, option_type = 'CE' } = req.body || {};
    if (!market_price || !spot || !strike) return res.json({ status: 'error', message: 'market_price, spot, and strike required' });
    const T = Math.max(days_to_expiry / 365, 1 / 365);
    const r = 0.07;
    const iv = calcImpliedVol(spot, strike, T, r, market_price, option_type);
    res.json({ status: 'success', implied_volatility: iv, market_price, spot, strike, days_to_expiry, option_type });
  });
  ...
  router.post('/api/ws/start', (req, res) => {
    res.json({ status: 'success', message: 'WebSocket streaming available in desktop app only' });
  });

  router.post('/api/ws/stop', (req, res) => {
    res.json({ status: 'success', message: 'WebSocket streaming available in desktop app only' });
  });
