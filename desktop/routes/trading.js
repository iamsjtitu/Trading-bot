  ...
  router.get('/api/trades/active', async (req, res) => {
    ...
    if (mode === 'LIVE' && accessToken) {
      ...
      const tradesFromPositions = positions.map(pos => {
        ...
        const qty = Math.abs(pos.quantity); const investment = entryPrice * qty; const currentPrice = pos.last_price || 0; const currentValue = currentPrice * qty; const livePnl = pos.pnl || (currentValue - investment);
        const stored = storedTradesMap[pos.instrument_token] || {};
        if (stored.id && entryPrice > 0 && (stored.entry_price === 0 || stored.entry_price === 150)) { stored.entry_price = entryPrice; stored.investment = investment; const slPct = customStoplossPct != null ? customStoplossPct : rp.stop_loss_pct; const tgtPct = customTargetPct != null ? customTargetPct : rp.target_pct; stored.stop_loss = Math.round(entryPrice * (1 - slPct / 100) * 100) / 100; stored.target = Math.round(entryPrice * (1 + tgtPct / 100) * 100) / 100; db.save(); }
        return { id: stored.id || pos.instrument_token, trade_type: pos.quantity > 0 ? 'BUY' : 'SELL', symbol: pos.trading_symbol || stored.symbol || 'N/A', quantity: qty, status: 'OPEN', entry_price: Math.round(entryPrice * 100) / 100, current_price: currentPrice, current_value: Math.round(currentValue * 100) / 100, investment: Math.round(investment * 100) / 100, live_pnl: Math.round(livePnl * 100) / 100, pnl_percentage: entryPrice > 0 ? Math.round(((currentPrice - entryPrice) / entryPrice) * 10000) / 100 : 0, stop_loss: stored.stop_loss || 0, target: stored.target || 0, entry_time: stored.entry_time || new Date().toISOString(), isLive: true, instrument_token: pos.instrument_token || '', product: pos.product || '', signal_id: stored.signal_id || '' };
      });
