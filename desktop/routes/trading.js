...
module.exports = function (db) {
  ...
  router.get('/api/trades/today', async (req, res) => {
    ...
    if (currentMode === 'LIVE') {
      ...
      for (const trade of allOpenPaper) {
        if (!trade._simPrice) trade._simPrice = trade.entry_price;
        const volatility = trade.entry_price * 0.0003;
  ...
  router.post('/api/auto-exit/check', async (req, res) => {
    ...
    for (const trade of openTrades) {
      ...
      if (shouldExit) {
        ...
        if (tgAlerts.trade_exit !== false) {
          try { const tg = require('./lib/telegram'); tg.sendTradeExitAlert(trade).catch(() => {}); } catch (_) {}
        }
  ...
  router.post('/api/trades/manual-exit', async (req, res) => {
    ...
    if (tgAlertsPaper.trade_exit !== false) {
      try { const tg = require('./lib/telegram'); tg.sendTradeExitAlert(trade).catch(() => {}); } catch (_) {}
    }
