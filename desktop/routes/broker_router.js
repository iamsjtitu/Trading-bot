...
module.exports = function (db) {
  ...
  // ==================== Market Data ====================
  router.get('/api/broker/market-data', async (req, res) => {
    const brokerId = getActiveBroker(db);
    const token = getToken(db, brokerId);
    if (!token) return res.json({ status: 'error', message: 'Not connected' });
    ...
    try {
      switch (brokerId) {
        case 'upstox': {
          const INDEX_KEYS = {
            nifty50: 'NSE_INDEX|Nifty 50', sensex: 'BSE_INDEX|SENSEX',
            banknifty: 'NSE_INDEX|Nifty Bank', finnifty: 'NSE_INDEX|Nifty Fin Service',
          ...
          if (resp.data?.status === 'success') {
            ...
            const data = {};
            for (const [key, instKey] of Object.entries(INDEX_KEYS)) {
              let quote = raw[instKey];
              if (!quote) { const namePart = instKey.split('|')[1] || ''; const matchKey = Object.keys(raw).find(k => k.includes(namePart)); if (matchKey) quote = raw[matchKey]; }
              if (!quote) { data[key] = { value: 0, change: 0, changePct: 0 }; continue; }
              const ltp = quote.last_price || 0;
              const netChange = quote.net_change || 0;
              const prevClose = ltp - netChange;
              data[key] = { value: ltp, change: Math.round(netChange * 100) / 100, changePct: prevClose > 0 ? Math.round((netChange / prevClose) * 10000) / 100 : 0 };
            }
        ...
        default:
          return res.json({ status: 'error', message: `Market data not available for ${BROKER_CONFIGS[brokerId]?.name || brokerId}` });
      }
  ...
  // ==================== P&L ====================
  router.get('/api/broker/pnl', async (req, res) => {
    const brokerId = getActiveBroker(db);
    const token = getToken(db, brokerId);
    if (!token) return res.json({ status: 'error', message: 'Not connected' });
    ...
    try {
      switch (brokerId) {
        case 'upstox': {
          const resp = await axios.get('https://api.upstox.com/v2/user/get-profit-and-loss-charges', {
            headers: { Accept: 'application/json', Authorization: `Bearer ${token}`, 'Api-Version': '2.0' },
            params: { segment: segment === 'FO' ? 'FO' : 'EQ' },
            timeout: 10000,
          });
          return res.json({ status: 'success', data: resp.data?.data || {}, broker: brokerId });
        }
        case 'zerodha': {
          const creds = getBrokerCreds(db, brokerId);
          ...
          const positions = resp.data?.data?.net || [];
          const totalPnl = positions.reduce((s, p) => s + (p.pnl || 0), 0);
          return res.json({ status: 'success', data: { total_pnl: Math.round(totalPnl * 100) / 100, positions: positions.length }, broker: brokerId });
        }
        default:
          return res.json({ status: 'error', message: `P&L not available for ${BROKER_CONFIGS[brokerId]?.name || brokerId}` });
      }
    } catch (e) {
      res.json({ status: 'error', message: e.message });
    }
